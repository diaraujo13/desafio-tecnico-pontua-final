...
import { approveVacationUseCase } from '../../../main/container';
import { ApproveVacationDTO } from '../../../application/dtos/VacationRequestDTO';
import { Result } from '../../../domain/shared/Result';
import { UnexpectedDomainError } from '../../../domain/errors/UnexpectedDomainError';

/**
 * Hook for approving a vacation request using TanStack Query Mutation
 *
...
export function useApproveVacation() {
  ...
  const mutation = useMutation({
    mutationFn: async (dto: ApproveVacationDTO): Promise<void> => {
      const result = await approveVacationUseCase.execute(dto);
  ...
  const approveVacation = async (dto: ApproveVacationDTO): Promise<Result<void>> => {
    try {
